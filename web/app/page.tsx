...
const GAMES = [
  { type: 'attention', name: 'Hawkeye', icon: EyeIcon, desc: 'Visual search & attention' },
  { type: 'memory', name: 'Target Tracker', icon: SparklesIcon, desc: 'Working memory & tracking' },
  { type: 'speed', name: 'Speed Reaction', icon: BoltIcon, desc: 'Processing speed & reaction' },
  { type: 'logic', name: 'Pattern Logic', icon: PuzzlePieceIcon, desc: 'Logic & pattern recognition' },
];
...
export default function Home() {
  ...
  const playGame = async (gameType: string, difficulty: number) => {
    setLoading(true);
    try {
      ...
      fetch('/api/session', {
        ...
        body: JSON.stringify({ user_id: userId, game_type: gameType, score: data.score, accuracy: data.accuracy, difficulty }),
      }).catch(() => {});
      setResult(data);
      loadStats();
      ...
      <div className="max-w-6xl mx-auto px-4 py-12">
                ...
                <div className="flex items-start gap-4">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-4 flex gap-2">
                      {[1, 2, 3].map(diff => (
                        <button key={diff} onClick={() => playGame(game.type, diff)} disabled={loading}
        ...
        {result && (
          <div className="bg-slate-800/50 backdrop-blur rounded-2xl p-6 border border-purple-500/20 mb-8">
            <h2 className="text-white font-semibold text-xl mb-4 flex items-center gap-2">
              <TrophyIcon className="w-6 h-6 text-yellow-400" /> Result
            </h2>
            <div className="grid grid-cols-3 gap-4">
              <div><div className="text-gray-400 text-sm">Score</div><div className={`text-3xl font-bold ${scoreColor(result.score)}`}>{result.score}</div></div>
              <div><div className="text-gray-400 text-sm">Accuracy</div><div className={`text-3xl font-bold ${accColor(result.accuracy)}`}>{result.accuracy.toFixed(1)}%</div></div>
              <div><div className="text-gray-400 text-sm">Response</div><div className="text-3xl font-bold text-white">{result.response_time.toFixed(0)}ms</div></div>
            </div>
          </div>
        )}
        ...
        {loading && (
          <div className="text-center py-12">
            <SparklesIcon className="w-16 h-16 text-purple-400 animate-spin mx-auto mb-4" />
            <p className="text-purple-200">Training...</p>
          </div>
        )}

        <div className="bg-slate-800/50 backdrop-blur rounded-2xl p-6 border border-purple-500/20">
          <h2 className="text-white font-semibold text-xl mb-4 flex items-center gap-2">
            <ClockIcon className="w-6 h-6 text-purple-400" /> Recent Sessions
          </h2>
          <div className="space-y-2">
            {sessions.length === 0 ? (
              <p className="text-gray-400 text-center py-4">No sessions yet. Start training!</p>
            ) : sessions.slice(0, 10).map((s) => (
              <div key={s.id} className="flex justify-between items-center p-3 bg-slate-900/50 rounded-lg">
                <div>
                  <span className="text-white font-medium">{s.game_type}</span>
                  <span className="text-gray-400 text-sm ml-2">Lvl {s.difficulty}</span>
                </div>
